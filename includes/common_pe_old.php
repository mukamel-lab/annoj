...
function get_boxes($table, $assembly, $left, $right, $seq)
{
	if ($seq)
	{
		$query = "select id, 'read', strand, start, end, length(sequenceA), length(sequenceB), sequenceA, sequenceB from $table where assembly='$assembly' and start <= $right and end >= $left order by start asc, end desc";
	}
	else
	{
		$query = "select id, 'read', strand, start, end, length(sequenceA), length(sequenceB), '', '' from $table where assembly='$assembly' and start <= $right and end >= $left order by start asc, end desc";
	}
