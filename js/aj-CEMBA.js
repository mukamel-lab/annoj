/*
 * ------------------------------------------------------------------------------
 * Modifications for the CEMBA project, December 2018, Eran Mukamel
 * ------------------------------------------------------------------------------
 * ------------------------------------------------------------------------------
...
// added uniqu function by H. C. 2015-12-21
...
//added setPolicy function by H. C. 2016-03-24
...
var AnnoJ = (function() {
	...
	function init() {
		...
		// EAM: create default values for the page info
		if (!AnnoJ.config.info) {
			AnnoJ.config.info={};
			AnnoJ.config.info.title={};
			AnnoJ.config.info.genome={};
			AnnoJ.config.info.contact={};
			AnnoJ.config.info.email={};
			AnnoJ.config.info.institution={};
		}
	...
	*/
	function buildTracks() {
		...
		function(trackConfig, index) {
			...
			//GUI.TrackSelector.expand();		//added by H. C 2015-11-30
			//GUI.TrackSelector.active.expand();	//added by H. C 2015-11-30
			//GUI.TrackSelector.inactive.expand()	//added by H. C 2015-11-30
		});
		GUI.TrackSelector.expand();			//added by H. C 2015-11-30
		GUI.TrackSelector.active.expand();		//added by H. C 2015-11-30
		GUI.TrackSelector.inactive.expand();		//added by H. C 2015-11-30
		...
		function(track) {
			if (!track.isProxy)			//added by H. C 2015-11-30
			{
				track.on('describe',
	...
	function buildGUI() {
		...
		var Bookmarker = new AnnoJ.Bookmarker({
			...
			//datasource: config.bookmarks || config.genome		//tmp added by H. C. 2015-07-14
		});
		...
		var ccj = '';						//added by H. C. 2015-06-22
		if (config.citation) {
			//AboutBox.addCitation(config.citation)		//added by H. C. 2015-06-22
			ccj += config.citation				//added by H. C. 2015-06-22
		}
		//below 7 lines added by H. C. 2015-06-22
		if (config.jbuilder) {
			//ccj += "<a href=http://neomorph.salk.edu/aj2/ target=_blank><div class='jbuilder'> &nbsp; </div></a>";
			ccj += "<div style=\"padding:5px;\">The page and its AnnoJ.Config are generated by using jBuilder " + config.jbuilder + " [H. C.] </div><br>";
		}
		if (ccj != '') {
			AboutBox.addCitation(ccj)
		}
		...
		Bookmarker.on('save', Bookmarker.save);		//added by H. C. 2015-09-20
		Bookmarker.on('load', Bookmarker.load);		//added by H. C. 2015-09-20
		//Bookmarker.disable(); 			//added by H. C. 2015-09-20
		//Bookmarker.hide();				//added by H. C. 2015-09-20
		...
		return {
			...
			Bookmarker: Bookmarker,	//test works
			InfoBox: InfoBox,
			AboutBox: AboutBox,
			NavBar: NavBar,
			Tracks: Tracks,
...
AnnoJ.TrackSelector = (function() {
	...
	function manage(track) {
		if (!track instanceof AnnoJ.BaseTrack && !track.isProxy) return; //added by H. C 2015-11-30
	...
	//next activatePath and inactivatePath functions added by H. C. 2014-11-20
	function inactivatePath(node)
	{
		...
		function(child) {
			...
			if ( child.originalParent==node) {
				inactivate(child.track);			//node.appendChild(child);
	...
	function importPath(path) {
		if (this.path == path) return this.parent;	//added by H. C 2015-11-30
		var dirs = path.split('/');
		var parent = inactive;
		Ext.each(dirs,
		function(dir) {
			...
			if (!child) {
				...
				parent.appendChild(child);	//added by H. C 2015-11-30
			}
			//parent.appendChild(child);		//added by H. C 2015-11-30
			parent = child
		});
		this.path = path;				//added by H. C 2015-11-30
		this.parent = parent;				//added by H. C 2015-11-30
		return parent
	};
	function getActive() {
		var list = [];
	...
	return function(userConfig) {
		...
		//dblclick function added by H. C.
...
AnnoJ.Navigator = function() {
	...
	var Controls = (function() {
		...
		var defaultSettings = {
			...
			scale: 2,
			multi: 1,
			activate: 0,					//added by H. C. 2015-06-19
			hic_d: 0,
			yaxis: 100					// changed by EAM 2016-08-24
		};
		...
		//next activateMode added by H. C. 2015-06-19
		if(AnnoJ.config.settings.activate == 1) checked1 = false;
		else checked1 = true;
		var activateMode = new Ext.CycleButton({
			showText: true,
			prependText: 'Activate: ',
			...
			changeHandler: function(btn, item) {
				if(item.text == "Single") AnnoJ.config.settings.activate = 0;
				else AnnoJ.config.settings.activate = 1;
			}
		});
		...
		var ensemble = new Ext.CycleButton({
			showText: true,
			prependText: 'Ensemble: ',
			tooltip: 'Ensemble of single cells to display',
			items: [{
					text: 'Ens10',
					checked: true
				},
			...
			changeHandler: function(btn, item) {
				// var newEns = item.text.replace(/^Ens/g,'');
				AnnoJ.config.ens = item.text.replace(/^Ens/g,'');
			}
		});
		...
		function refreshControls() {
			...
			// ensemble.setValue(AnnoJ.config.ens);
			closer.enable();
			further.enable();
		...
		else setSelected(scaleMode, 'Fixed(Genome)');
		//added by H. C. 2015-06-19 next two lines with activateMode
		if(AnnoJ.config.settings.activate == 1) setSelected(activateMode, 'Folder');
		else setSelected(activateMode, 'Single');
		scaleBox.setValue(1);
		...
		return {
			...
			scaleMode: scaleMode,
			showMode: showMode,
			activateMode: activateMode,			//added by H. C. 2015-06-19
			scaleBox: scaleBox,
			Baseline: Baseline,
	...
	var Toolbar = new Ext.Toolbar({
		...
		//added by H. C. 2015-06-19 next two lines '-', Controls.activateMode
		// Modified by EAM and CK 2017
		items: [ 
		// Controls.ensemble , '  |  ', 
		 Controls.dragMode, '  |  ', 'Zoom level: ',Controls.ratio, Controls.further, Controls.closer, '  |  ', 'Chromosome:',Controls.assembly,'Position:', Controls.jump, Controls.go,'  |  ', Controls.prev, Controls.next, '  |  ', Controls.scaleMode]
	...
	var ehandler = function(method){
		...
		if(method == 'Correlation' || method == 'Intensity'){
			...
			BaseJS.request({
				...
				success: function(response) {
					...
					trk.setLocation(loc);
					//trk.Toolbar.setScale(trackConfig.scale);	//not work properly
				},
		...
		if(method == 'Correlation' || method == 'Intensity'){
			...
			AnnoJ.getGUI().TrackSelector.insertBefore(trk, first.config.name);
			trk.setLocation(loc);
			//trk.Toolbar.setScale(trackConfig.scale, true);	//Not work properly
			AnnoJ.getGUI().Tracks.tracks.refresh();
		}
	};
...
AnnoJ.Bookmarker = (function() {
	...
	var button = new Ext.Button({
		...
		handler: function() {
			var name;
			if (!input.isValid()) {
				name = AnnoJ.config.markname;			//added by H. C. 2015-10-11
				//WebApp.warning('Please enter a bookmark name');
				//return					//added by H. C. 2015-10-11
			}
			else name = input.getValue();				//added by H. C. 2015-10-11
			//add(input.getValue());				//added by H. C. 2015-10-11
			add(name);						//added by H. C. 2015-10-11
			save()							//added by H. C. 2015-07-14
		}
	});
	...
	function save() {
		...
		BaseJS.request({
			url: server.datasource,
			method: 'POST',
			requestJSON: false,				//added by H. C. 2015-07-14
			data: {
				action: 'bookmarks_save',		//debug by H. C. 2015-07-14
				bookmarks: JSON.stringify(bookmarks)	//debug by H. C. 2015-07-14
			},
			success: function(response) {
				AnnoJ.notice(response.message, false)	//debug by H. C. 2015-07-14
			},
			failure: function(response) {
				AnnoJ.error(response)
			}
	...
	function redraw() {
		body.update('');					//debug by H. C. 2015-07-14
	...
	function remove(bookmark) {
		...
		Ext.each(bookmarks,
		function(item) {
			if (item != bookmark) {				//debug by H. C. 2015-07-14
				clean.push(item)
			}
		});
		delete bookmark;
	...
	return function(url) {
		...
		server = url || '';
		this.load = load()					//debug by H. C. 2015-07-14
	}
})();
...
AnnoJ.AboutBox = (function() {
	var info = {
		//logo: " <a href='http://www.annoj.org' target=_blank><div class='annoj'> &nbsp; </div></a>",
...
AnnoJ.Tracks = function(userConfig) {
	...
	this.Region = (function() {
		...
		function() {
			...
			if (dragMode == 'browse') {
				...
				var mov = mouse.y-mouse.downY;		//added by H. C 2015-11-07
				body.dom.scrollTop -= mov;		//added by H. C 2015-11-07
				self.tracks.each(function(track) {
					track.moveCanvas(0, 0);		//added by H. C 2015-11-30
					//track.moveCanvas(0);		//added by H. C 2015-11-30
			...
			if (dragMode == 'zoom') {
				...
				var loc = AnnoJ.getLocation();
				hide();
				//next 33 lines added by H. C. 2015-10-25
				//if (width < 10) return;
				if (width < 10)
				{
					...
					//var b = loc.bases; var p = loc.pixels;
					var closer = (z > 0) ? 1 : 0;
		...
		function() {
			...
			if (dragMode == 'browse') {
				...
				ext.setTop(mouse.y - mouse.downY);	//added by H. C 2015-11-30
				self.tracks.each(function(track) {
	...
	this.tracks = (function() {
		...
		function manage(track) {
			if (!track instanceof AnnoJ.BaseTrack && !track.is.Proxy) return; //added by H. C 2015-11-30
			if (AnnoJ.isReady && isManaged(track)) return; 		//added by H. C 2015-11-30
			tracks.push(track);
			if (track instanceof AnnoJ.BaseTrack){			//added by H. C 2015-11-30
		...
		function myclose(track) {			//added by H. C. 2015-11-05
			if (!isActive(track)) return;
			active.remove(track);
...
var PointList = function() {
	...
	this.insert = function(node) {
		...
		if (node.x <= first.x) {
			...
			//next 6 lines added by H. C. Save at least 10% of memory
	...
	this.insertPoints = function(array) {
		...
		if (len == 0) return; //added by H. C 2015-11-30
	...
	this.viewport = (function() {
		...
		function set(x1, x2) {
			...
			if (x1 > x2) return;
			for (var node = first; node; node = node.next) {
				//if (node.x < x1) continue; 			//added by H. C 2015-10-09
		...
		function update(x1, x2) {
			...
			//while (viewL && viewL.prev && viewL.prev.x >= x1) {   //added by H. C. 2015-10-09
...
var RangeList = function() {
	...
	this.viewport = (function() {
		...
		function set(x1, x2) {
			...
			viewL = null;
			viewR = null;
			//for (var node = firstR; node; node = node.nextR) {			//added by H. C. 2013-09-20
			for (var node = firstL; node; node = node.nextL) {			//added by H. C. 2013-09-20
		...
		function update(x1, x2) {
			...
			while (viewL && viewL.prevL && viewL.prevL.x2 >= x1) {			//added by H. C. 2013-09-19
				viewL = viewL.prevL
			}
	...
	this.parse = function(data, above) {
		...
		function(datum) {
			...
			if (datum.length == 3) {
				var item = {
					x: parseInt(datum[0]),
					w: AnnoJ.pixels2bases(1),	//parseInt(b),
	...
	this.parse = function(data, above) {
		...
		for (var name in data) {
			...
			function(datum) {
				...
				read.sequence += "," + read.copies;				//added by H. C. 2013-09-25
				if (read.id && read.x && read.w && read.places && read.copies) {
					if (read.places > 1) read.cls += ' multi_mapper';
					if (read.copies > 1) read.cls += ' multi_copies';
					switch (read.w) {
	...
	this.subset2canvas = function(x1, x2, bases, pixels) {
		...
		self.viewport.apply(function(node) {
			...
			var item = {
				...
				w: Math.round(node.value.w * pixels / bases) || boxWidthMin || 1,
				children: []
			};
			if (node.value.children) {
				for (var id in node.value.children) {
					...
					if (cw < boxWidthMin) cw = boxWidthMin;		//added by H. C. 2015-09-26
					if (cw) {
						item.children.push({
							id: child.id,
							cls: child.cls,
...
var HistogramCanvas = function() {
	...
	this.paint = function() {
		...
		for (var cls in data) {
			...
			if (id.substring(0,4)=='new-') {			//next 8 lines added by H. C. 2015-11-06
				var nc = nc || 0;
			...
			//var track = findConf(id);
			if(!track.color) track.color = {};			//debug by H. C. 2015-10-14
			else if (typeof(track.color) == 'string')		//debug by H. C. 2015-10-14
				track.color =  Ext.decode(track.color);		//debug by H. C. 2015-10-14
			if(track.color[cls]){
				...
				if (track.color['rColor'] && track.color['rColor']!='0')	//added by H. C.
					cc = brush.fillStyle;					//added by H. C.
...
var MaskCanvas = function(userConfig) {
	...
	this.paint = function() {
		...
		if (container.style.display=='none') return ;			//added by H. C. 2015-06-20
...
var AlignsCanvas = function(userConfig) {
	...
	this.paint = function() {
		...
		var nl = 0; var nr = 0; var xl = 0; var xr = 0;
		var h = height;
...
var ModelsCanvas = function(userConfig) {
	...
	this.paint = function() {
		...
		//var sequence = AnnoJ.config.genome;					//added by H. C.
		var sequence = AnnoJ.config.sequence;					//added by H. C.
		var ratio = AnnoJ.config.genome_ratio;					//added by H. C.
		...
		//added by H. C. 2013-09-18 (self.config.name.indexOf('Gene Models') != -1)
...
var ReadsCanvas = function(userConfig) {
	...
	this.paint = function() {
		...
		//else level += copies - 1; 					//removed by H. C. 2013-09-25
		...
		for (var i = 0; i < copies; i++) {
			...
			{						//below two lines added by H. C. 2015-08-28
				if(lane.indexOf("AJ_below") != -1)
			...
			if(sequence) {
				if(lane.indexOf("AJ_below") != -1){
					...
					//letterize(brush, seq, x, y - i*(h + self.config.boxSpace), w, h, container)	//removed by H. C. 2015-08-28
					letterize(brush, seq, x, y + i*(h + self.config.boxSpace), w, h, container)	//added by H. C. 2015-08-28
				}
				else
					letterize(brush, sequence, x, y - i*(h + self.config.boxSpace), w, h, container);
			}
		}
		level += copies - 1;					   //added by H. C. 2015-08-28
...
//SNPsCanvas  -- added by H. C. 2016-06-19
var SNPsCanvas = function(userConfig) {
	...
	this.paint = function() {
		...
		function(read) {
			...
			var lane = brush.canvas.parentNode.className;
			var letterW = 6; //AnnoJ.bases2pixels(1);

			if (read.cls=='Ins') self.paintBox(read.cls, x-3, y, 9, h);
			...
			if(sequence) {
				...
				letterize(brush, seq, x2, y+h+1, w, h, container)     //added by H. C. 2015-08-28
			}
			else
...
AnnoJ.BaseTrack = function(userConfig) {
	...
	this.Toolbar = (function() {
		...
		//added by H. C 2015-11-30 prevWalk, setWalk, nextWalk
		...
		if(isInfo(self.config.id) || self.config.id == 'trackyyyy-0'){
			...
			if (self.config.showWalks)
				items = [closeButton, title, filler, prevWalk, setWalk, nextWalk, spacer, val, spacer, scale_box, toggleButton, spacer];
...
AnnoJ.BrowserTrack = function(userConfig) {
	...
	var DataManager = (function() {
		...
		function setLocation(requested) {
			...
			if(self.config.type == "ModelsTrack" && ratio >= 5 && !isInfo(self.config.id))
			{
				...
				if((!AnnoJ.config.genome_left || !AnnoJ.config.genome_right || AnnoJ.config.genome_left != left || AnnoJ.config.genome_right != right || views.requested.assembly != state.assembly))
				{
					...
					if (self.config.name.indexOf('Gene Models') != -1)
		...
		function loadGenome(gleft, gright, name) {
			var gurl;
			//if(name.indexOf('hg18') != -1) gurl = "/proxy/http://tabit.ucsd.edu/fetchers/hg18_genome.php";  //added by H. C.
			//if(name.indexOf('hg19') != -1) gurl = "/proxy/http://tabit.ucsd.edu/fetchers/hg19_genome.php";  //added by H. C.
			//gurl = AnnoJ.config.genome;						//added by H. C.
			//gurl = "/aj2/genomes/arabidopsis_thaliana.php";			//added by H. C.
			gurl = AnnoJ.config.genome;						//added by H. C.
			state.busy = true;
			...
			BaseJS.request({
				...
				data: {
					action: 'range',
					table: 'genome',					//added by H. C. 2015-09-14
					assembly: views.loading.assembly,
					left: gleft,						//added by H. C.
					right: gright,						//added by H. C.
					bases: policy.bases,
				...
				success: function(response) {
					...
					if(response.data[0] == "GENOME"){
						//AnnoJ.config.genome = response.data[2];	 		//added by H. C.
						AnnoJ.config.sequence = response.data[2]; 			//added by H. C.
		...
		function loadFrame(frame) {
			...
			if(self.config.id != 'trackyyyy-0' && self.config.id != 'trackxxxx-0'){
				...
				sources = self.config.urls;	//track.urls;
				if(self.config.type == 'HiCTrack')
				sources = AnnoJ.config.location.assembly + ',' + track.assembly;
				actions = self.config.action;	//track.action;
				if(!sources) sources = '';
				if(!actions) actions = '';
				//alert(track.actions);
				tables = self.config.name;	//track.name
			}
			if(self.config.type == 'HiCTrack'){
	...
	this.close = function() {
		if (self.config.action2)				//added by H. C. 2015-10-25
		{
			self.mask('Track Closed');			////added by H. C. 2013-09-26
			DataManager.clear();				////added by H. C. 2013-09-26
			self.remove()					////added by H. C. 2013-09-26
		}
		else self.hide()					//added by H. C. 2013-09-26
	};
	this.hide = function() {
		self.mask('Track Closed');
		DataManager.clear();
		self.Frame.ext.setHeight(0);
		self.Frame.ext.setVisibilityMode(Ext.Element.DISPLAY);		//added by H. C. 2015-07-04 1:24 AM
...
AnnoJ.MaskTrack = function(userConfig) {
	...
	if (self.config.single) {
		...
		containerA.setStyle('borderBottom', 'solid lightgray 1px')
	}
...
AnnoJ.ModelsTrack = function(userConfig) {
	...
	var defaultConfig = {
		...
		showLabels: true,
		showArrows: true,
		showWalks: true,	//added by H. C 2015-11-30
		labelPos: 'left',
		alignControl: 0,
		boxHeight: 10,
		boxHeightMax: 24,
	...
	if (self.config.searchBox) {
		...
		var search = new Ext.form.ComboBox({
			...
			onSelect: function(record) {
				...
				loc.position = parseInt(record.data.tss); // EAM 4/2019
				AnnoJ.setLocation(loc);
				self.fireEvent('browse', loc);
				this.collapse()
			}
	...
	var Models = (function() {
		...
		function paint(left, right, bases, pixels) {
			...
			if(subsetA.length > 0){
				...
				if (!self.config.markname)			//added by H. C. 2015-11-13
					self.config.markname = subsetA[i].id;	//added by H. C. 2015-11-13
			} else if(subsetB.length > 0){
				var i = Math.floor(subsetB.length/2);
				AnnoJ.config.markname = subsetB[i].id;
				if (!self.config.markname)			//added by H. C. 2015-11-13
					self.config.markname = subsetB[i].id;	//added by H. C. 2015-11-13
			}
		...
		BaseJS.request({
			...
			success: function(response) {
				if (response.success) {
					...
					AnnoJ.config.markname = d.id;		//added by H. C 2015-11-30
					self.config.markname = d.id;		//added by H. C 2015-11-30
				} else {
...
AnnoJ.MethTrack = function(userConfig) {
	...
	var defaultConfig = {
		single: false,
		class: 'CG CHG CHH',				// added by H. C. 2015-12-21
		clsAbove: 'AJ_above',
		clsBelow: 'AJ_below'
	};
	Ext.applyIf(self.config, defaultConfig);
	...
	if (self.config.showborder) {
		containerA.setStyle('borderTop', 'solid silver 2px');
	}
	if (self.config.single) {
		containerA.setStyle('height', '100%');
		containerB.setStyle('display', 'none')
	...
	// added next 39 lines and removed old 70 lines by H. C. 2015-12-21
	var menuItems = new Array();
	var toolbarItems = new Array();
	var classItems = ['silk_bullet_black', 'silk_bullet_green', 'silk_bullet_red', 'silk_bullet_orange', 'silk_bullet_blue', 'silk_bullet_pink' ];
	var classes = self.config.class.split(' ');

	// added next 9 lines by H. C. 2016-06-20
	var hideClasses = [];
	...
	// below next 9 lines added by H. C. 2016-06-20
...
AnnoJ.PairedEndTrack = function(userConfig) {
	...
	var Histogram = (function() {
		...
		function parse(data) {
			for (var series in data) {
				addLabel(series);
				self.Toolbar.addItem(series);	//added by HC 2015-09-14
			}
			dataA.parse(data, true);
			dataB.parse(data, false);
			if(Ext.isGecko){
...
//AnnoJ.AlignsTrack  -- added by H. C. 2015-09-21
AnnoJ.AlignsTrack = function(userConfig) {
	...
	Ext.applyIf(self.config, userConfig || {}, defaultConfig);
	var containerA = new Ext.Element(document.createElement('DIV'));
	var containerB = new Ext.Element(document.createElement('DIV'));
	containerA.addClass(self.config.clsAbove);
	containerB.addClass(self.config.clsBelow);
...
//AnnoJ.SNPsTrack  -- added by H. C. 2016-06-19
